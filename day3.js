...
coordArray1.map((obj) => {
    let i;
    for(i=0; i < coordArray2.length; i++) {
        let Xcoord = coordArray2[i].x;
        let Ycoord = coordArray2[i].y;
        let stepsCoord = coordArray2[i].steps
        
        if (obj.x === Xcoord && obj.y === Ycoord) {
            let distanceObj = {
                distance: Math.abs(Xcoord) + Math.abs(Ycoord),
                steps: obj.steps + stepsCoord
            }
...
distanceArray.sort(function(a, b){
    return Math.abs(0-a.steps) - Math.abs(0-b.steps);
});

console.log(distanceArray[0].steps);